...
_DEFAULT_LOG_FILE = "~/.kyle/logs/core.log"
_DEFAULT_LOG_FORMAT = "text"
_DEFAULT_CONFIG_PATH = "~/.kyle/config.toml"
_DEFAULT_MAX_STEPS = 20
_DEFAULT_MODEL = "claude-sonnet-4-6"
_DEFAULT_TRACE_FILE = "~/.kyle/traces/daemon.jsonl"
_DEFAULT_LLM_PROVIDER = "anthropic"
_DEFAULT_IPC_TOKEN_FILE = "~/.kyle/ipc-token"
...
def get_config() -> KyleConfig:
    config = KyleConfig()

    # .env 必须在读取 KYLE_CONFIG 之前加载，以便 .env 中的 KYLE_CONFIG 能影响 TOML 路径
    load_dotenv(".env", override=False)

    # 若显式指定 KYLE_CONFIG，只读该文件；否则按优先级叠加：全局 → 项目本地
    explicit = os.environ.get("KYLE_CONFIG")
    if explicit:
        config_paths = [Path(explicit).expanduser()]
    else:
        config_paths = [
            Path(_DEFAULT_CONFIG_PATH).expanduser(),
            Path(".kyle/config.toml"),
        ]
...
def _apply_toml(config: KyleConfig, data: dict[str, Any]) -> None:
    known_sections = {
...
# 用 KYLE_* 环境变量覆盖 config 中对应字段（若变量已设置）
def _apply_env(config: KyleConfig) -> None:
    host = os.environ.get("KYLE_HOST")
    if host is not None:
        config.host = host

    port_str = os.environ.get("KYLE_PORT")
    if port_str is not None:
        try:
            config.port = int(port_str)
        except ValueError:
            raise SystemExit(f"Config error: KYLE_PORT must be an integer, got: {port_str!r}")

    ipc_token_file = os.environ.get("KYLE_IPC_TOKEN_FILE")
    if ipc_token_file is not None:
        if not ipc_token_file.strip():
            raise SystemExit("Config error: KYLE_IPC_TOKEN_FILE must not be empty")
        config.ipc_token_file = ipc_token_file

    log_level = os.environ.get("KYLE_LOG_LEVEL")
    if log_level is not None:
        config.logging.level = log_level

    log_file = os.environ.get("KYLE_LOG_FILE")
    if log_file is not None:
        config.logging.file = log_file

    log_format = os.environ.get("KYLE_LOG_FORMAT")
    if log_format is not None:
        config.logging.format = log_format

    max_steps_str = os.environ.get("KYLE_MAX_STEPS")
    if max_steps_str is not None:
        try:
            val = int(max_steps_str)
            if val <= 0:
                raise SystemExit(
                    "Config error: KYLE_MAX_STEPS must be a positive integer,"
                    f" got: {max_steps_str!r}"
                )
            config.agent.max_steps = val
        except ValueError:
            raise SystemExit(
                f"Config error: KYLE_MAX_STEPS must be an integer, got: {max_steps_str!r}"
            )

    default_model = os.environ.get("KYLE_LLM_DEFAULT_MODEL")
    if default_model is not None:
        config.llm.default_model = default_model

    llm_provider = os.environ.get("KYLE_LLM_PROVIDER")
    if llm_provider is not None:
        config.llm.provider = llm_provider

    llm_base_url = os.environ.get("KYLE_LLM_BASE_URL")
    if llm_base_url is not None:
        config.llm.base_url = llm_base_url

    llm_api_key_env = os.environ.get("KYLE_LLM_API_KEY_ENV")
    if llm_api_key_env is not None:
        config.llm.api_key_env = llm_api_key_env

    trace_enabled = os.environ.get("KYLE_TRACE_ENABLED")
    if trace_enabled is not None:
        config.trace.enabled = trace_enabled.lower() not in ("0", "false", "no")

    trace_file = os.environ.get("KYLE_TRACE_FILE")
    if trace_file is not None:
        config.trace.file = trace_file

    trace_payload = os.environ.get("KYLE_TRACE_INCLUDE_LLM_PAYLOAD")
    if trace_payload is not None:
        config.trace.include_llm_payload = trace_payload.lower() not in ("0", "false", "no")

    trace_event_payload = os.environ.get("KYLE_TRACE_INCLUDE_PAYLOAD")
    if trace_event_payload is not None:
        config.trace.include_payload = trace_event_payload.lower() not in ("0", "false", "no")

    trace_max_bytes = os.environ.get("KYLE_TRACE_MAX_BYTES")
    if trace_max_bytes is not None:
        try:
            ...
            raise SystemExit(
                "Config error: KYLE_TRACE_MAX_BYTES must be a non-negative integer, "
                f"got: {trace_max_bytes!r}"
            ) from None

    trace_backup_count = os.environ.get("KYLE_TRACE_BACKUP_COUNT")
    if trace_backup_count is not None:
        try:
            ...
            raise SystemExit(
                "Config error: KYLE_TRACE_BACKUP_COUNT must be a non-negative integer, "
                f"got: {trace_backup_count!r}"
            ) from None

    perm_timeout = os.environ.get("KYLE_PERMISSION_TIMEOUT_S")
    if perm_timeout is not None:
        try:
            perm_timeout_val = float(perm_timeout)
            if perm_timeout_val < 0:
                raise SystemExit(
                    f"Config error: KYLE_PERMISSION_TIMEOUT_S must be >= 0, got: {perm_timeout!r}"
                )
            config.permission.timeout_s = perm_timeout_val
        except ValueError:
            raise SystemExit(
                f"Config error: KYLE_PERMISSION_TIMEOUT_S must be a number, got: {perm_timeout!r}"
            )

    compact_threshold = os.environ.get("KYLE_COMPACT_THRESHOLD")
    if compact_threshold is not None:
        try:
            compact_threshold_val = float(compact_threshold)
            if not (0.0 <= compact_threshold_val <= 1.0):
                raise SystemExit(
                    "Config error: KYLE_COMPACT_THRESHOLD must be between 0 and 1, "
    ...
    if compact_tool_limit is not None:
        try:
            ...
            if compact_tool_limit_val <= 0:
                raise SystemExit(
                    "Config error: KYLE_COMPACT_TOOL_LIMIT must be a positive integer, "
                    f"got: {compact_tool_limit!r}"
                )
            config.compaction.tool_result_limit = compact_tool_limit_val
        except ValueError:
            raise SystemExit(
                "Config error: KYLE_COMPACT_TOOL_LIMIT must be an integer, "
                f"got: {compact_tool_limit!r}"
            )

    compact_tool_keep = os.environ.get("KYLE_COMPACT_TOOL_KEEP")
    if compact_tool_keep is not None:
        try:
            compact_tool_keep_val = int(compact_tool_keep)
            if compact_tool_keep_val <= 0:
                raise SystemExit(
                    "Config error: KYLE_COMPACT_TOOL_KEEP must be a positive integer, "
