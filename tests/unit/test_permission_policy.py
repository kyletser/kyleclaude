...
def test_safe_tools_default_allow() -> None:
    ...
    assert evaluate("checkpoint_list", {}) == PermissionDecision.ALLOW
    assert evaluate("list_dir", {"path": "."}) == PermissionDecision.ALLOW
    assert evaluate("glob", {"pattern": "**/*.py"}) == PermissionDecision.ALLOW
    assert evaluate("git_diff", {"path": "."}) == PermissionDecision.ALLOW
    assert evaluate("grep", {"pattern": "Session"}) == PermissionDecision.ALLOW
...
def test_write_file_default_is_ask() -> None:
    assert evaluate("apply_patch", {"patch": "--- a/x"}) == PermissionDecision.ASK
    assert evaluate("checkpoint_rewind", {}) == PermissionDecision.ASK
    assert evaluate("write_file", {"path": "out.txt", "content": "hi"}) == PermissionDecision.ASK
    assert evaluate("edit_file", {"path": "out.txt"}) == PermissionDecision.ASK
...
def test_param_preview_known_tools() -> None:
    assert param_preview("apply_patch", {"patch": "--- a/x"}) == "patch='--- a/x'"
    assert param_preview("bash", {"command": "echo hi"}) == "command='echo hi'"
    assert param_preview("read_file", {"path": "README.md"}) == "path='README.md'"
    assert param_preview("edit_file", {"path": "src/app.py"}) == "path='src/app.py'"
    assert param_preview("note_save", {"content": "Python 3.12"}) == "content='Python 3.12'"
    assert param_preview("glob", {"pattern": "**/*.py"}) == "pattern='**/*.py'"
    assert param_preview("git_diff", {"path": "."}) == "path='.'"
    assert param_preview(
        "checkpoint_rewind", {"checkpoint_id": "20260716T120000-12345678"}
    ) == "checkpoint_id='20260716T120000-12345678'"
    assert param_preview("grep", {"pattern": "Session"}) == "pattern='Session'"
