...
# 设计：BaseTool 默认 retry_policy=NEVER，确保写入、Shell、外部调用不会重复副作用
async def test_default_tool_policy_never_retries(monkeypatch: pytest.MonkeyPatch) -> None:
    class _SideEffectTool(BaseTool):
        name = "side_effect"
        description = "simulates a non-idempotent operation"
        ...
        async def invoke(self, params: dict[str, object]) -> ToolResult:
            self.calls += 1
            return ToolResult("failed", is_error=True, error_type="runtime_error")
