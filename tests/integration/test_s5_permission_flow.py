...
async def test_headless_fail_fast_stops_run_without_waiting(tmp_path: Path) -> None:
    manager = PermissionManager()
    manager.set_session_mode("", "fail_fast")
    bus = EventBus()
    ...
    assert getattr(failed, "error_class", "") == "permission_required"


async def test_headless_deny_returns_tool_error_but_allows_replanning(tmp_path: Path) -> None:
    manager = PermissionManager()
    manager.set_session_mode("", "deny")
    bus = EventBus()
    events: list[BaseModel] = []
    ...
    assert outcome.status == "success"
    assert "tool.call_finished" not in [getattr(event, "type", "") for event in events]
    failed = next(event for event in events if getattr(event, "type", "") == "tool.call_failed")
